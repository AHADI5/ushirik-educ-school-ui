...
import { IconButton, Select, MenuItem, FormControl, InputLabel, Button, CircularProgress } from '@mui/material';
...
// Simulated API call to fetch the current titular teacher
const getCurrentTitularTeacher = async (classID) => {
  ...
  return teacher; // Assuming the first teacher is the current titular
};
...
export default function Overview() {
  ...
  useEffect(() => {
    async function fetchCurrentTeacher() {
      const teacher = await getCurrentTitularTeacher(classID);
  ...
  const handleAssignTeacher = async (event) => {
    ...
    try {
      ...
      setIsLoading(false);
      setShowSelectBox(false); // Hide the select box after assignment
    }
  };
